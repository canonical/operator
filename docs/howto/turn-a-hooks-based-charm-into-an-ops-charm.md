...
Note how `relation.data` provides an interface to the relation databag (see [](/howto/manage-relations)) and we need to select which part of that bag to access by passing an `ops.model.Unit` instance.
