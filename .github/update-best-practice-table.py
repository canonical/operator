...
as a stable identifier for the practice. The generated output emits each item as
a Sphinx anchor (``(<name>)=``) followed by the bullet line, so consumers can
deep-link to a specific practice, and downstream automation can match practices
by ID via a trailing ``<!-- id: <name> -->`` HTML comment.
...
# can locate per-practice identifiers.
HEADER = """\
<!--
...
Each practice that declares ``:name: <slug>`` in its source admonition is
emitted as a Sphinx anchor ``(<slug>)=`` followed by the bullet line, and is
tagged with a trailing HTML comment ``<!-- id: <slug> -->`` so that downstream
automation (e.g. charmhub-listing-review) can match practices by ID. The
``(<slug>)=`` anchor is omitted when the admonition lives on the same page as
the ``{include}`` of this file, because the admonition's own ``:name:`` already
provides that anchor on the host page.

Do not edit this file directly; update the source admonition instead.
-->
"""

# Regex used to find pages that ``{include}`` this generated file. Such pages
# already render the source admonitions inline, so emitting ``(<slug>)=`` for
# their bullets would create duplicate Sphinx targets.
INCLUDE_HOST_RE = re.compile(r'\{include\}[^\n]*best-practices\.txt')


def find_include_hosts(docs_dir: pathlib.Path) -> set[pathlib.Path]:
    """Return doc files that ``{include}`` the generated best-practices file."""
    hosts: set[pathlib.Path] = set()
    for path in docs_dir.rglob('*'):
        if path.suffix not in ('.md', '.rst'):
            continue
        if INCLUDE_HOST_RE.search(path.read_text()):
            hosts.add(path)
    return hosts
...
def main():
    ...
    include_hosts = find_include_hosts(path_to_ops / 'docs')
    for directory, base_url, make_ref, ref_sub in (
        ...
        for file_path in sorted(directory.rglob('*')):
            ...
            is_include_host = file_path in include_hosts
            if len(practices):
                print(f'**[{title}]({link})**')
            for heading, ref, practice, name in practices:
                ...
                if name:
                    # Skip the anchor on pages that include this file directly,
                    # since the source admonition's :name: already provides it
                    # on the same page (a duplicate target would warn).
                    if not is_include_host:
                        print(f'({name})=')
