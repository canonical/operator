...
Each practice that declares a :name: slug in its source admonition is
emitted as a Sphinx anchor (slug)= on the line before the bullet, and the
bullet is tagged with a trailing HTML id-comment containing the slug so
that downstream automation (e.g. charmhub-listing-review) can match
practices by ID. The (slug)= anchor is omitted when the admonition lives
on the same page as the include of this file, because the admonition's
own :name: already provides that anchor on the host page.
