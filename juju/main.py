...
def _setup_hooks(charm_dir, charm):
    ...
    emitters = [charm] + list(charm.relations.values()) + list(charm.storage.values())
    for emitter in emitters:
        for bound_event in emitter.on.events().values():
...
def _emit_charm_event(charm, hook_name):
    ...
    if '-relation-' in hook_name:
        ...
        emitter = charm.relations[relation_name].on
