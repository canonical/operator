...
REEMITTING_EVT_REGEX_NEW = re.compile(
    f"Re-emitting deferred event {_EVENT_REPR}"
)  # ops >= 2.1
...
def match_line(line: str) -> Optional[EventEmissionLog]:
    if grps := JUJU_EVT_REGEX.findall(line):
        ...
        REEMITTING_EVT_REGEX_OLD.findall(line) or REEMITTING_EVT_REGEX_NEW.findall(line)
    ):
        _type_name, source, name, _id = grps[0]
...
def emitted_events(state: "State") -> List[EventEmissionLog]:
    ...
    """
    return [match for _, line in state.juju_log if (match := match_line(line))]
