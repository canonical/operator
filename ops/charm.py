...
class SecretChangedEvent(SecretEvent):
    """Event triggered by Juju on the observer when the secret owner changes its contents.
...
class SecretRotateEvent(SecretEvent):
    """Event triggered by Juju on the owner when the secret's rotation policy elapses.
...
class SecretRemoveEvent(SecretEvent):
    """Event triggered by Juju on the owner when a secret revision can be removed.
...
class SecretExpiredEvent(SecretEvent):
    """Event triggered by Juju on the owner when a secret's expiration time elapses.
...
class CharmEvents(ObjectEvents):
    ...
    This class is used to create an event descriptor (``self.on``) for
    ...
    By default, the events listed below will be provided via the
    :attr:`CharmBase.on` attribute. For example::

        self.framework.observe(self.on.config_changed, self._on_config_changed)

    In addition to the events listed below, dynamically-named events will also
    be defined based on the charm's metadata (``metadata.yaml``) for relations,
    storage, actions, and containers. These named events may be accessed as
    ``self.on[<name>].<event>`` or using a prefix like ``self.on.<name>_<event>``,
    for example::
