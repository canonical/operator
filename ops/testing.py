...
class ActionOutput:
    ...
    The results dictionary is flattened to a :code:`dict[str, str]` when communicating with
    Juju, and Juju restores it back to a multi-level dict. This attribute is the non-flat
    version, so may be a multi-level dict, and calling :meth:`ops.ActionEvent.set_results`
    may raise :class:`ValueError` if the results cannot be flattened.

    Multiple calls to :meth:`ops.ActionEvent.set_results` update the dictionary rather
    than replacing it.
    """
