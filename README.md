...
- Harness exposes an imperative API: the user is expected to call methods on the Harness driving it to the desired state, then assert that everything looks right (typically by calling certain charm methods directly, or verifying the content of the mock filesystem).
- Harness instantiates the charm once, then allows you to fire multiple events on the charm, which is breeding ground for subtle bugs. Scenario tests are centered around testing single state transitions, that is, one event at a time (modulo deferred events). This ensures that the execution environment is as clean as possible (for a unit test), and much closer to that of a 'real' Juju unit.
