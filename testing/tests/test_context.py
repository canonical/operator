...
def test_context_tmp_cleanup():
    """Test that the Context.__del__ properly cleans up the temporary directory.

    This test verifies that:
    1. The temp directory is created and exists after Context creation
    2. The temp directory persists after run() calls
    3. No ResourceWarning is triggered when the context is garbage collected
    4. The temp directory is cleaned up after __del__ is called
    """
    import pathlib
    import warnings
    ...
    # Run a scenario to ensure the temp dir is used and persists after run()
    ctx.run(ctx.on.start(), State())

    # Temp directory should still exist after run()
    assert tmp_path.exists()

    # Call __del__ directly because cyclic references prevent garbage collection
    # from calling it reliably.
    with warnings.catch_warnings():
        warnings.simplefilter('error', ResourceWarning)

        ctx.__del__()

    # Verify the temp directory is cleaned up after __del__ is called
