...
def test_status_history_manager(mycharm):
    class StatusCharm(mycharm):
        def __init__(self, framework):
            super().__init__(framework)
            framework.observe(self.on.update_status, self._on_update_status)
...
def test_status_history_preservation_manager(mycharm):
    class StatusCharm(mycharm):
        def __init__(self, framework):
            super().__init__(framework)
            framework.observe(self.on.update_status, self._on_update_status)
    ...
    assert out.unit_status == WaitingStatus('3')
    assert mgr.unit_status_history == [ActiveStatus('foo')]

    assert out.app_status == WaitingStatus('3')
    assert mgr.app_status_history == [ActiveStatus('bar')]
...
def test_workload_history_manager(mycharm):
    class WorkloadCharm(mycharm):
        def __init__(self, framework):
            super().__init__(framework)
            framework.observe(self.on.install, self._on_install)
    ...
    with ctx(ctx.on.install(), State(leader=True)) as mgr:
        out = mgr.run()
    assert out.workload_version == '1'
    assert mgr.workload_version_history == []

    with ctx(ctx.on.start(), out) as mgr:
        out = mgr.run()
    assert out.workload_version == '1.1'
    assert mgr.workload_version_history == ['1']

    with ctx(ctx.on.update_status(), out) as mgr:
        out = mgr.run()
    # Note that the context manager is fresh for each run, so it doesn't have '1' here.
    assert mgr.workload_version_history == ['1.1']
    assert out.workload_version == '1.2'
