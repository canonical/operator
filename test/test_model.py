...
""")
    ...
    def test_can_connect_connection_error(self):
        ...
        assert cm.output[0].search(r'DEBUG:ops.model:.*: connection error!')
    ...
    def test_can_connect_file_not_found_error(self):
        ...
        with self.assertLogs('ops', level='DEBUG') as cm:
            assert not self.container.can_connect()
        assert len(cm.output) == 1
        assert cm.output[0].search(r'DEBUG:ops.model:.*: file not found!')
    ...
    def test_can_connect_api_error(self):
        ...
        with self.assertLogs('ops') as cm:
            assert not self.container.can_connect()
        assert len(cm.output) == 1
        assert cm.output[0].search(r'WARNING:ops.model:.*: api error!')
...
class TestPorts(unittest.TestCase):
    ...
    def test_opened_ports_warnings(self):
        ...
        assert cm.output[0].search(r'WARNING:ops.model:.*protocol.*')
        assert cm.output[1].search(r'WARNING:ops.model:.*range.*')
