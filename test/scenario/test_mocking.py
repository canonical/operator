...
from scenario.structs import State, Scene, Event, _DCBase, relation
from typing import Dict, Tuple, Any, Callable


def mock_simulator(fn: Callable,
                   namespace: str,
                   tool_name: str,
                   scene: "Scene",
                   call_args: Tuple[Any, ...],
...
def test_patch_generic_module(mock_foo):
    ...
    scene = Scene(state=state.copy(),
                  event=Event('foo'))

    from test.scenario.resources import demo_decorate_class
    patch_module(
        demo_decorate_class,
        {
            "MyDemoClass": {
                "get_foo": DecorateSpec(
                    simulator=mock_simulator
                ),
                "set_foo": DecorateSpec(
                    simulator=mock_simulator
                )
            }
        },
    ...
    from test.scenario.resources.demo_decorate_class import MyDemoClass
