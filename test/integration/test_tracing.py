...
def test_direct_connection(build_charm: Callable[[], str], juju: jubilant.Juju):
    ...
    spans = wait_spans(
        ...
        ready=lambda spans: 'ops.main' in str(spans),
    )
    ...
    spans = wait_spans(
        status.apps['tempo-worker'].address,
        ready=lambda spans: 'one_action' in str(spans),
        since=checkpoint,
    )

    # A span generated by ops
    assert 'one_action: TracingTesterCharm' in [span['name'] for span in spans]
    # A span generated by the test charm
    assert 'on action' in [span['name'] for span in spans]

    action_span = next(span for span in spans if span['name'] == 'on action')
...
def test_with_tls(build_charm: Callable[[], str], juju: jubilant.Juju):
    ...
    juju.integrate('tracing-tester', 'self-signed-certificates')
    juju.wait(jubilant.all_active)

    juju.integrate('tracing-tester', 'tempo')
    status = juju.wait(jubilant.all_active)
    print('incomplete test, continue here', status)
