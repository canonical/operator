...
class TestRecursive(unittest.TestCase):
    ...
    def test_list_recursive(self):
        cases = [
                  TestRecursive.Case(
                      path='/', max_depth=10, max_files=10, follow=False,
                  ...
                  TestRecursive.Case(
                      path='/', max_depth=0, max_files=10, follow=False,
                      files=['/foo/bar.txt', '/baz.txt'],
                      wantlist=['/baz.txt'],
                    ),
                  TestRecursive.Case(
                      path='/', max_depth=10, max_files=1, follow=False,
                      files=['/foo/bar.txt', '/baz.txt'],
                      wantlist=['/foo/bar.txt'],
                    ),
                  TestRecursive.Case(
                      path='/', pattern='bar.*', max_depth=10, max_files=10, follow=False,
                      files=['/foo/bar.txt', '/baz.txt'],
                      wantlist=['/foo/bar.txt'],
                    ),
                  TestRecursive.Case(
                      path='/', pattern='bar.*', max_depth=10, max_files=10, follow=False,
                      files=['/foo/bar.txt', '/bar.txt'],
                      wantlist=['/foo/bar.txt', '/bar.txt'],
                    ),
                  TestRecursive.Case(
                      path='/', pattern='foo/*', max_depth=10, max_files=10, follow=False,
                      files=['/foo/bar.txt', '/bar.txt'],
                      wantlist=['/foo/bar.txt'],
                    ),
                ]
        ...
        def list_func_gen(file_list):
            args = {'last_modified': datetime.time(), 'permissions': 0o777, 'size': 42, 'user_id': 0, 'user': 'foo', 'group_id': 1024, 'group': 'bar'}
            file_infos, dirs = [], set()
            for f in file_list:
                file_infos.append(pebble.FileInfo(path=f, name=os.path.basename(f), type=pebble.FileType.FILE, **args))

                # collect all the directories for the test case's files
                dirpath = os.path.dirname(f)
                if dirpath != '' and dirpath not in dirs:
                    dirs.update(dirpath)
                    file_infos.append(pebble.FileInfo(path=dirpath, name=os.path.basename(dirpath), type=pebble.FileType.DIRECTORY, **args))
            ...
            def inner(path):
                ...
                for info in file_infos:
                    ...
                    if not info.path.startswith(path) or path == info.path:
                        continue
        ...
        for i, case in enumerate(cases):
            files = pebble._list_recursive(list_func_gen(case.files), case.path, case.pattern, case.follow, case.max_depth, case.max_files)
            self.assertEqual(case.wantlist, [f.path for f in files], msg='case {} failed'.format(i+1))
