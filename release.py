...
def get_new_version_post_release(repo: github.Repository.Repository, branch_name: str) -> str:
    ...
    For final releases, the value is generated by bumping the minor version of the
    latest release's tag and adding the '.dev0' suffix.

    For pre-releases (alpha, beta, rc), the next dev version uses the same base version
    with '.dev0' suffix, since we're still working towards that final release.
    """
    ...
    if version.pre is not None:
        logger.info('Latest version is a pre-release, using base version for next dev version.')
